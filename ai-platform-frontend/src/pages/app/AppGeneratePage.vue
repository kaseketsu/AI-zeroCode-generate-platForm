...
import { type ElementInfo, VisualEditor } from '@/utils/visualEdit.ts'
...
const loadChatHistory = async (isLoadMore = false) => {
  ...
  try {
    ...
    if (isLoadMore && lastCreateTime.value) {
      params.lastGenerateTime = lastCreateTime.value
    }
...
const updatePreview = () => {
  if (appId.value) {
    const codeGenType = appInfo.value?.codeGenType || codeGenTypeEnums.HTML
    const newPreviewUrl = getStaticBaseUrl(codeGenType, appId.value)
...
const deleteApp = async () => {
  ...
  try {
    const res = await deleteAppById({ id: appInfo.value.id })
    if (res.data.code === 0) {
